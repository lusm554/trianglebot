...
def getFigure(f, coors) -> str:
    ...
    img = os.path.join(
        ASSETS_DIR,
        '{}.{}'.format(datetime.timestamp(datetime.now()), IMG_EXT)
    )
    if f == 'triangle':
        f = Figure('triangle')
        q = Queue()
...
def start(update, context) -> None:
    '''Sends a message with information about commands.'''
    header = ('Hey! My name is triangle and I can create triangles!'
              'U can share me with friend by @createtrianglebot\n'
              'Here is a list of my commands:\n')
    print(header)
    context.bot.send_message(
            chat_id=update.effective_chat.id,
            text=cmd_info(header)
    )
...
def main() -> None:
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            ARGUMENTS: [
                MessageHandler(
                    Filters.regex('[a|b|c]=[0-9]+ [a|b|c]=[0-9]+ [a|b|c]=False'),
                    coordinates
                )
            ]
        },
    ...
    # Run the bot until user exit from process. i.e. presses Ctrl-C or
